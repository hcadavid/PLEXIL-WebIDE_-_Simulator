...
public class PlexilExecLauncher {
    ...
    public void killProcess(long pid) throws ProcessKillException{
        String cmd1 = "/bin/bash";
        ProcessBuilder pb = new ProcessBuilder(cmd1, "-c", "kill -15 " + pid);

        Process p;
        try {
            p = pb.start();
        } catch (IOException ex) {
            throw new ProcessKillException("Error while killing process "+pid,ex);
        }
        ...
        try {
            ...
            if (r != 0) { // No error                
                throw new ProcessKillException("Unable to kill process "+pid+". command returned "+r);
            }
        ...
        try {
            ...
            if (!errorOutcomeMsg.equals("")){
                throw new ProcessKillException("Process Kill Error:"+errorOutcomeMsg);
            }
        } catch (InterruptedException ex) {
            throw new ProcessKillException("Process Kill Error:"+ex,ex);
        }
    ...
    public static void main(String args[]) throws IOException {
        try {
            PlexilExecLauncher.getInstance().killProcess(Long.parseLong(args[0]));
